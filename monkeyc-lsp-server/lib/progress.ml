...
(*             | None -> *)
(*               (1* This can happen when we connect to dune in the middle of a *)
(*                  build. *1) *)
(*               start_build t *)
(*           in *)
(*           let total = complete + remaining in *)
(*           (1* The percentage is useless as it isn't monotinically increasing as *)
(*              the spec requires, but it's the best we can do. *1) *)
(*           let percentage = *)
